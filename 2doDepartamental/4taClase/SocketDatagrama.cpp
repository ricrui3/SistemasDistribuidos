...
int SocketDatagrama::recibe(PaqueteDatagrama &p) {
  ...
  if ((tam = recvfrom(s, p.obtieneDatos(), p.obtieneLongitud(), 0,
                      (struct sockaddr *)&direccionForeanea, &n)) <= 0)
...
int SocketDatagrama::envia(PaqueteDatagrama &p) {
  ...
  // cout << "--------------\n"<<p.obtieneDatos();
  int tam;
  if ((tam = sendto(s, p.obtieneDatos(), p.obtieneLongitud(), 0,
                    (struct sockaddr *)&direccionForeanea,
                    sizeof(direccionForeanea))) == -1)
